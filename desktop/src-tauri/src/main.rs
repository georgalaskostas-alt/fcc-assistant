...
fn expected_backend_is_healthy()->bool{
    // Do not treat an arbitrary process on 8765 as FCC Assistant. This exact
    // HTTP probe verifies the service identity before a dev relaunch reuses it.
    let Ok(mut stream)=TcpStream::connect_timeout(&SocketAddr::from_str("127.0.0.1:8765").unwrap(),Duration::from_millis(300)) else{return false};
    ...
    if backend_is_listening(){
        if expected_backend_is_healthy(){
            if let Ok(mut s)=diagnostics.lock(){
                s.last_error=None;
                s.recent_output.push("startup: verified FCC backend already listening on 8765; reusing existing local backend".into());
            }
        }else if let Ok(mut s)=diagnostics.lock(){
            s.last_error=Some("Port 8765 is occupied by a different local service. FCC backend was not spawned.".into());
            s.recent_output.push("startup: foreign service detected on 8765; FCC backend identity check failed".into());
        }
