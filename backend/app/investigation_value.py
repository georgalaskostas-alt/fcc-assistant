...
def rank_measurement_candidates(*, candidates: list[dict[str, Any]], hypothesis: dict[str, Any],
                                resolved_tags: list[str] | None = None) -> list[dict[str, Any]]:
    """Rank governed measurement candidates by expected information value.

    This is deliberately deterministic and metadata-only: it does not invent a
    process mechanism. Search relevance, semantic/label overlap with the active
    hypothesis, and novelty determine which authorized historian read is worth
    spending the next bounded tool call on.
    """
    resolved = {str(value).casefold() for value in (resolved_tags or [])}
    focus = str(hypothesis.get("statement") or "").casefold()
    focus_tokens = {token for token in focus.replace("_", " ").replace("-", " ").split() if len(token) >= 3}
    ranked: list[dict[str, Any]] = []
    for index, row in enumerate(candidates):
        if not isinstance(row, dict):
            continue
        ...
        semantic = str(row.get("semantic_key") or row.get("semantic") or "")
        aliases = row.get("aliases") if isinstance(row.get("aliases"), (list, tuple)) else []
        searchable = " ".join([key, label, semantic, *map(str, aliases)]).casefold().replace("_", " ").replace("-", " ")
        overlap = sum(1 for token in focus_tokens if token in searchable)
        raw_relevance = row.get("score", row.get("relevance", 0.0))
        try:
            relevance = float(raw_relevance or 0.0)
        except (TypeError, ValueError):
            relevance = 0.0
        score = 5.0 + min(max(relevance, 0.0), 1.0) * 3.0 + min(overlap, 4) * 0.75
        ranked.append({
            "tag_key": key,
            "score": round(score, 3),
            "catalog_rank": index,
            ...
            "reason": "governed_catalog_candidate",
        })
    return sorted(ranked, key=lambda item: (-item["score"], item["catalog_rank"], item["tag_key"]))
