    ...
    for round_index in range(max(0, max_rounds)):
        ...
        if branch_allocation:
            ...
            for action in plan.get("actions") or []:
                branch_id=str(action.get("hypothesis_branch_id") or "") if isinstance(action,dict) else ""
                if branch_id and branch_id in branch_allocation:
                    if branch_used[branch_id]>=branch_allocation[branch_id]:continue
                    branch_used[branch_id]+=1
