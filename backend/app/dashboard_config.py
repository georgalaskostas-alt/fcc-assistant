...
def _mentioned_units(text: str, site: SiteModel) -> list[ProcessUnit]:
    ...
    for unit in site.units:
        positions = [
            position
            for candidate in (unit.key.casefold(), unit.name.casefold())
            if candidate and (position := text.find(candidate)) >= 0
        ]
        if positions:
...
def _build_widgets_for_unit(
    ...
    if widget_type == "summary":
        return [
            asdict(
                DashboardWidget(
                    id=f"{unit.key}-summary-summary",
                    type="summary",
                    title=f"{unit.name} Summary",
                    unit_key=unit.key,
                    tag_keys=(),
                    period=period,
                    layout=_default_layout("summary", start_order),
                )
            )
        ]
    ...
    if widget_type == "trend":
        ...
        return [
            asdict(
                DashboardWidget(
                    id=f"{unit.key}-trend-{'-'.join(tag_keys)}",
                    type="trend",
                    title=" / ".join(tag.label for tag in resolved),
                    unit_key=unit.key,
                    tag_keys=tag_keys,
                    period=period,
                    layout=_default_layout("trend", start_order),
                )
            )
        ]
    ...
    for offset, tag in enumerate(resolved):
        ...
        created.append(
            asdict(
                DashboardWidget(
                    id=f"{unit.key}-{widget_type}-{tag.key}",
                    type=widget_type,
                    title=title,
                    unit_key=unit.key,
                    tag_keys=(tag.key,),
                    period=period,
                    layout=_default_layout(widget_type, start_order + offset),
                )
            )
        )
    return created
...
def _clone_unit_widgets(
    source: ProcessUnit,
    targets: list[ProcessUnit],
        ...
        for source_widget in source_widgets:
            ...
            for source_tag_key in source_tag_keys:
                ...
                if source_tag is None:
                    missing_semantics.append(source_tag_key)
                    continue
                target_tag = target.tag_by_semantic(source_tag.semantic)
                if target_tag is None:
                    missing_semantics.append(source_tag.semantic)
                    continue
                target_tag_keys.append(target_tag.key)
                target_labels.append(target_tag.label)

            if source_tag_keys and not target_tag_keys:
                warnings.append(f"{target.name}: could not map {', '.join(missing_semantics)}")
                continue
            ...
            if widget_type == "summary":
                title = f"{target.name} Summary"
                suffix = "summary"
            elif widget_type == "trend":
                title = " / ".join(target_labels)
                suffix = "-".join(target_tag_keys)
            elif widget_type == "average":
                title = f"Average {target_labels[0]}" if target_labels else f"{target.name} Average"
                suffix = target_tag_keys[0] if target_tag_keys else "average"
            else:
                title = target_labels[0] if target_labels else target.name
                suffix = target_tag_keys[0] if target_tag_keys else "kpi"

            source_layout = source_widget.get("layout")
            if isinstance(source_layout, dict):
                layout = WidgetLayout(
                    order=len(current_widgets) + len(cloned),
                    width=int(source_layout.get("width", 6)),
                    height=str(source_layout.get("height", "normal")),
                )
            else:
                layout = _default_layout(widget_type, len(current_widgets) + len(cloned))  # type: ignore[arg-type]
            ...
            cloned.append(
                asdict(
                    DashboardWidget(
                        ...
                        title=title,
                        unit_key=target.key,
                        tag_keys=tuple(target_tag_keys),
                        period=str(source_widget.get("period", "8h")),
                        layout=layout,
                    )
                )
            )
...
def plan_dashboard_command(
    ...
    clone_requested = _contains_any(
        text,
        ("τα ίδια", "τα ιδια", "το ίδιο", "το ιδιο", "ίδιο με", "ιδιο με", "same as", "same for", "copy"),
    )
    if clone_requested and len(mentioned_units) >= 2:
        source = mentioned_units[0]
        targets = mentioned_units[1:]
        ...
        return {
            "action": "add_widgets",
            "widgets": cloned,
            "source_unit": source.key,
            "target_units": [unit.key for unit in targets],
            "warnings": warnings,
    ...
    is_layout_command = _contains_any(text, (
        "μετακίνησε", "μετακινησε", "τοποθέτησε", "τοποθετησε",
    ...
    for unit in mentioned_units:
        ...
        if unit_widgets:
            planned_widgets.extend(unit_widgets)
        else:
            unresolved_units.append(unit.name)
    ...
    if len(planned_widgets) == 1:
        result.update({"action": "add_widget", "widget": planned_widgets[0]})
    else:
        result.update({"action": "add_widgets", "widgets": planned_widgets})
    return result
