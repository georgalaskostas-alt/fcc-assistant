...
def plan_follow_up(*, goal: str, unit_key: str, synthesis: dict[str, Any], hypotheses: list[dict[str, Any]], max_actions: int = 3) -> dict[str, Any]:
    ...
    if not archive.get("count") or statuses.intersection({"independent_evidence_available","relevant_but_insufficient","insufficient_independent_evidence"}):
        actions.append({"tool":"search_archive","reason":"Need more specific approved technical evidence","arguments":{"query":goal,"unit_key":unit_key,"approved_only":True,"limit":8}})
    if not events.get("count"):
        actions.append({"tool":"search_alarms_events","reason":"Need independent event evidence","arguments":{"unit_key":unit_key,"query":goal,"limit":100}})
    ...
    return {
        ...
        "actions": selected,
        "bounded_by": {"max_actions": max_actions},
