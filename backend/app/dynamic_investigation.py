...
class DynamicInvestigator:
    ...
    async def investigate(self, *, goal: str, unit_key: str, context: ToolContext) -> DynamicInvestigationResult:
        ...
        combined["autonomous_investigation"] = await run_autonomous_evidence_loop(
            registry=self.registry, context=context, goal=goal, unit_key=unit_key,
            synthesis=combined, time_window={"start": intent.start_time, "end": intent.end_time},
            episode_context=episode_context, max_rounds=3)
