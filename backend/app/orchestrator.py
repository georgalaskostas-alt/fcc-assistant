...
class AssistantOrchestrator:
    ...
    async def analyze_shift(
        ...
        try:
            report = await self.shift_reports.generate(
                start_time=start_time,
                end_time=end_time,
                tag_keys=tag_keys,
            )
            response = await self.local_ai.generate(
                user_prompt=question,
                context={
                    "analysis_scope": "shift_report",
                    "start_time": start_time,
                    "end_time": end_time,
                    "report": report,
                },
        ...
        return AssistantAnswer(
            ...
            evidence_type="shift_report",
        )

    async def analyze_tag_period(
        self,
        question: str,
        tag_key: str,
        start_time: str,
        ...
        try:
            ...
            response = await self.local_ai.generate(
                user_prompt=question,
                context={
                    "analysis_scope": "single_tag_period",
                    "tag_key": tag_key,
                    "start_time": start_time,
                    "end_time": end_time,
                    "evidence": evidence,
        ...
        return AssistantAnswer(
            answer=response.text,
            model=response.model,
            evidence_type="single_tag_period",
        )
